    ...
    async with aiosqlite.connect('my_database.db') as db:
        ...
        async with db.execute("SELECT id FROM users WHERE id = ?", (user_id,)) as cursor:
            ...
            if not user_exists:
                ...
                await db.execute("INSERT INTO users (id, username, is_blocked,plus) VALUES (?, ?, 0,1)", (user_id, username))
                await db.commit()
...
async def send_statistics(message: types.Message):
    ...
    async with aiosqlite.connect('my_database.db') as db:
        ...
        # Подсчитываем общее количество объявлений в диапазоне, если есть объявления
        if min_id is not None and max_id is not None:
            async with db.execute("SELECT COUNT(*) FROM advertisements WHERE id BETWEEN ? AND ?",
                                  (min_id, max_id)) as cursor:
...
async def send_reply_to_user(message: types.Message, state: FSMContext):
    ...
    sent_message = await bot.send_message(reply_to_user_id, "Ваш вопрос был отвечен. Если хотите завершить диалог, обязательно нажмите кнопку ниже.", reply_markup=generate_cancel_support_button())
    ...
    async with aiosqlite.connect('my_database.db') as db:
        ...
        try:
            ...
            await bot.send_message(callback_query.from_user.id,
                                   f"Срок размещения объявления 14 дней, удачи в поисках!\n\nОбязательно нажмите кнопку назад чтобы пользоваться другими функциями бота",
...
async def view_ads(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not ads:
        ...
        await bot.send_message(
            callback_query.from_user.id,
            "В данном городе пока нет доступных объявлений. Разместите объявление первым!!!\nОбязательно нажмите кнопку назад чтобы в дальнейшем\nпользоваться другими командами",
...
async def send_ads_batch(user_id, state: FSMContext):
    ...
    if new_index < len(ads):
        ...
        await bot.send_message(user_id, "Вы просмотрели все доступные объявления в этом городе.\n\nОбязательно нажмите на кнопку назад чтобы пользоваться\nдругими функциями бота", reply_markup=generate_clear_chat_button())
